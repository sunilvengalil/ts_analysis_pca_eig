...
file_name = f"non_stochastic_linear_map"
M = 10
tau = 20
shuffle = False
...
if shuffle:
    svd_results_folder = f"svd_results_m_{M}_tau_{tau}_shuffled_synthetic_non_stochastic_lm"
    pca_results_folder = f"pca_results_m_{M}_tau_{tau}_shuffled_synthetic_non_stochastic_lm"
else:
    svd_results_folder = f"svd_results_m_{M}_tau_{tau}_synthetic_non_stochastic_lm"
    pca_results_folder = f"pca_results_m_{M}_tau_{tau}_synthetic_non_stochastic_lm"
...
ground_truths = ["NON-STOCHASTIC"] * len(files)
...
distinct_labels = [LABEL_STOCHASTIC]
