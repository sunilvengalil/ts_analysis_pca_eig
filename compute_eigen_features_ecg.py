...
"""ECG Data"""
file_names = ["Resamplerate121997b", "Resamplerate052099b", "Resamplerate011498a", "Resamplerate101397b"]
predictions =  ["STOCHASTIC", "NON-ST", "NON-ST", "STOCHASTIC" ]
gt_dictionary = {file_name:ground_truth for file_name, ground_truth in zip(file_names, ground_truths) }
predictions_dictionary = {file_name:prediction for file_name, prediction in zip(file_names, predictions) }

# file_name = "time_series"

file_name = "ecg_intervals"
M = 2
tau = 20
shuffle = False
...
if shuffle:
    ...
    svd_results_folder = f"eeg_svd_results_m_{M}_tau_{tau}_v2"
    pca_results_folder = f"eeg_pca_results"
...
#    values["Ground Truth"].append(gt_dictionary[key.capitalize()])
    values["Predictions"].append(predictions_dictionary[key.capitalize()])
...
labels = [1] * len(files)
for i, gt in enumerate(df["Predictions"].values):
    if gt == "STOCHASTIC":
        labels[i] = 2
